...
const LeaderboardPage = () => {
  ...
  useEffect(() => {
    // Fetch data and set allBatch and allSubject state
    fetch(
      `https://e-exam-pro-server.vercel.app/leaderboardResult?inputSearch=${searchInput}&batch=${selectedBatch}&subject=${selectedSubject}&sort=${selectedSort}&type=${selectedType}`
    )
      .then((res) => res.json())
      .then((data) => {
        console.log(data);
        ...
        <div className="mx-2">
          {examData?.map((data, index) => (
            <div
              ...
              className={
                selectedSort
                  ? "grid items-center grid-cols-2 md:grid-cols-7 gap-4 p-5 primary-bg my-3 shadow-lg rounded-xl"
                  : "grid items-center grid-cols-2 md:grid-cols-9 gap-4 p-5 primary-bg my-3 shadow-lg rounded-xl"
              }
            >
              <img
              ...
              <h1>
                ...
                <span className="text-yellow-500">{data.stu_email}</span>
              </h1>
              <h1>
                <span className="text-green-500">Name:</span>{" "}
                <span className="text-yellow-500">{data.stu_name}</span>
              </h1>
              <h1>
                <span className="text-green-500">Subject:</span>{" "}
                <span className="text-yellow-500">{data.subject}</span>
              </h1>
              {!selectedSort && (
                <h1 className="text-green-500">
                  <span>Code:</span>{" "}
                  <span className="text-yellow-500">{data.exam_code}</span>
                </h1>
              )}

              <h1>
                <span className="text-green-500">Batch:</span>{" "}
                <span className="text-yellow-500">{data.batch}</span>
              </h1>
              <h1>
                <span className="text-green-500">Type:</span>{" "}
                <span className="text-yellow-500">{data.examType}</span>
              </h1>
              <div>
                <h1>
                  <span className="text-green-500">Total Mark:</span>{" "}
                  <span className="text-yellow-500">{data.totalMark}</span>
                </h1>
                {!selectedSort && (
                  <h1>
                    <span className="text-green-500">Mark:</span>{" "}
                    <span className="text-yellow-500">{data.mark}</span>
                  </h1>
                )}
              </div>
              ...
              {!selectedSort && (
                <h1>
                  ...
                  <span className="text-yellow-500">{data.date}</span>
                </h1>
              )}
            </div>
          ))}
