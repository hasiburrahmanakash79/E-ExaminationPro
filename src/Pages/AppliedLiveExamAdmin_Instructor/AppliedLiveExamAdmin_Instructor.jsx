...
const AppliedLiveExamAdmin_Instructor = () => {
    ...
    const { data: appliedExam, refetch, isLoading } = useQuery({
        ...
        enabled: !loading,
        queryFn: async () => {
            const res = await axiosSecure.get(`appliedLiveExam?examID=${examID}&instructor_email=${instructor_email}`);
            return res.data;
        }
    })
    ...
    return (
        <>{
            ...
            ?<div className='text-red-400 text-4xl flex justify-center items-center h-[70vh]'>
                <p className='mx-2'><FaFaceFrown></FaFaceFrown> </p>
                <h1 ><Typewriter speed={200} delay={900} loop={true} text= {` No one Applied....`} /></h1></div>
            :
