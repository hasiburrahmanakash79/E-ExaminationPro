    ...
    if (!loading) {
      fetch(
        `https://e-exam-pro-server.vercel.app/questionPaper?type=${
          type || 'mcq'
        }&subject=${subject}&instructor_email=${user?.email}`
      )
        .then(res => res.json())
        .then(data => {
      ...
      <Tabs>
        <div className='text-center'>
          <TabList>
            ...
            <Tab onClick={() => dispatch(dispatch(examType('FillInTheBlank')))}>
              Fill In The Blank Exam
            </Tab>
            <Tab onClick={() => dispatch(dispatch(examType('multimedia_mcq')))}>
        ...
        <TabPanel>
          {exams.length == 0 ? (
            <div className='text-red-600 text-xl mt-5'>
              <h1>No Exam Added By You</h1>
            </div>
          ) : (
            <h2>
              {exams?.map((exam, index) => (
                <div key={index} className='my-2 card p-5 flex justify-center'>
                  <div className='grid grid-cols-2'>
                    ...
                    <div className='grid grid-cols-1'>
                      ...
                      {isInstructor ? (
                        <button className='btn w-1/3 ms-auto btn-sm btn-primary'>
